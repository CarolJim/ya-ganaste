...
public class PreferUserActivity extends LoaderActivity implements OnEventListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UsuarioResponse usuarioClienteResponse = SingletonUser.getInstance().getDataUserUyu().getUsuario();
        ClienteResponse ClienteResponse = SingletonUser.getInstance().getDataUserUyu().getCliente();
        ...
        if (SingletonUser.getInstance().getDataUserUyu().getEmisor()
                .getCuentas().get(0).getTarjetas() != null
                && !SingletonUser.getInstance().getDataUserUyu().getEmisor()
                .getCuentas().get(0).getTarjetas().isEmpty()) {
            mTDC = SingletonUser.getInstance().getDataUserUyu().getEmisor()
                    .getCuentas().get(0).getTarjetas().get(0).getNumero();
        } else {
            mTDC = "Error Con Tarjeta";
        }
        //mClabe = "123456789012345678";
        mClabe = SingletonUser.getInstance().getDataUserUyu().getEmisor()
        ...
        if (!SingletonUser.getInstance().getDataUserUyu().getEmisor().getCuentas().get(0).getTarjetas().equals(""))
            checkDataCard();
